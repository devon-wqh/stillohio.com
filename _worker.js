...
function readScreeningInput(body) {
  const s = {
    ...
    ticket_price: (body.ticket_price || '').trim() || null,
...
async function createScreening(request, env) {
  ...
  const res = await env.DB.prepare(
    `INSERT INTO screenings
      (title, display_date, display_date_end, sort_date, time, town, venue, badge, lat, lng, cta_type, ticket_url, ticket_block, ticket_price, status, tickets_sold, gross_cents)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).bind(
    s.title, s.display_date, s.display_date_end, s.sort_date, s.time, s.town, s.venue, s.badge,
    s.lat, s.lng, s.cta_type, s.ticket_url, s.ticket_block, s.ticket_price, s.status, s.tickets_sold, s.gross_cents
  ).run();
...
async function updateScreening(request, env, id) {
  ...
  await env.DB.prepare(
    `UPDATE screenings SET
      ...
      lat = ?, lng = ?, cta_type = ?, ticket_url = ?, ticket_block = ?, ticket_price = ?, status = ?, tickets_sold = ?, gross_cents = ?
     WHERE id = ?`
  ).bind(
    s.title, s.display_date, s.display_date_end, s.sort_date, s.time, s.town, s.venue, s.badge,
    s.lat, s.lng, s.cta_type, s.ticket_url, s.ticket_block, s.ticket_price, s.status, s.tickets_sold, s.gross_cents, id
  ).run();
  return json({ ok: true });
}
